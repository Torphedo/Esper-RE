...
bool alr_validate(std::string& msg, const al::editor& alr, bool headless) noexcept {
    if (!alr.loaded) {
        return true; // Not a failure, just not loaded
    }

    bool result = true;
    std::optional<al::editor::chunk> header_chunk;
    for (const al::editor::chunk& chunk : alr.chunks) {
...
bool alr_chunk_validate(const al::editor& alr, const al::editor::chunk& chunk, std::string& msg, bool headless) noexcept {
    ...
    switch (chunk.id) {
        ...
        case 0x5: {
            ...
            const al::editor::chunk skel_chunk = alr.first_chunk_in_range(3, chunk.offset, alr.alr_size);
