...
static inline double ease(int easetype, double t) {
    ...
    const double half_pi = 3.1415926535897932384626433832795 / 2;
...
#undef tween$bounceout
    ...
    case TYPE::SCHUBRING2: {
        const double p1pass = 2 * (p + (0.5f - p) * abs(0.5f - p)) - 0.5f;
        const double p2pass = 2 * (p1pass + (0.5f - p1pass) * abs(0.5f - p1pass)) - 0.5f;
        const double pAvg = (p1pass + p2pass) / 2;
        return pAvg;
    }
...
float CurveValue(float p, int maxpoints, const ImVec2* points) {
    ...
    const float d = (p - points[left].x) / (points[left + 1].x - points[left].x);
...
int Curve(const char* label, const ImVec2& size, const int maxpoints, ImVec2* points, int* selection, const ImVec2& rangeMin = ImVec2(0, 0), const ImVec2& rangeMax = ImVec2(1, 1)) {
    ...
    if (hovered) {
        ...
        const ImVec2 pos = ImRemap(hoverPos, ImVec2(0, 0), ImVec2(1, 1), rangeMin, rangeMax);
    ...
    if (hovered || draggingPoint) {
        ...
        for (int i = 0; i < pointCount; i++) {
            ...
            const ImVec2 a = p - ImVec2(4, 4);
            const ImVec2 b = p + ImVec2(4, 4);
            if(hoveredPoint == i) {
    ...
    if (hovered || draggingPoint) {
        ...
        snprintf(buf, sizeof(buf) - 1, "%s (%.2f,%.2f)", label, pos.x, pos.y);
        str = buf;
    }
