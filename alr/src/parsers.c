...
static void chunk_skip(arena_t* arena) {
    uint32_t size = *(uint32_t*)(arena_pos(arena) + sizeof(uint32_t));
...
static void chunk_0x0(arena_t* arena) {
    uint32_t size = *(uint32_t*)(arena_pos(arena) + sizeof(uint32_t));
    if (size != 8) {
        log_error(DEBUG, "chunk_0x0(): Anomaly! Size was %d bytes rather than the usual 8 bytes\n", size);
    }
    chunk_skip(arena);
}
...
static void chunk_0xD(arena_t* arena) {
    ...
    if (size != 0xC) {
        ...
        log_error(WARNING, "chunk_0xD(): Chunk is not empty! Size is %d bytes rather than the usual 12 bytes\n", size);
    }
    chunk_skip(arena);
}
